...
cloneCode() {
  if type git >/dev/null 2>&1; then
    ...
    if [[ git_clone == 0 ]]; then
      echo 'info: git clone java-alone success'
    fi
  else
...
cloneBuild() {
  ...
  echo 'info: clone java-alone'
  cloneCode

  echo 'info: build java-alone'
  cd java-tron
...
startService() {
  ...
  checkPid
  echo "info: start java-alone with pid $pid on $HOSTNAME"
