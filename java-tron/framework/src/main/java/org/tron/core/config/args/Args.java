...
public class Args extends CommonParameter {
  ...
  public static void clearParam() {
    ...
    PARAMETER.alnExpirationTimeInMilliseconds = 0;
  ...
  public static void setParam(final String[] args, final String confFileName) {
    ...
    PARAMETER.netMaxAlnPerSecond = config.hasPath(Constant.NODE_NET_MAX_ALN_PER_SECOND)
        ? config.getInt(Constant.NODE_NET_MAX_ALN_PER_SECOND)
        : NetConstants.NET_MAX_ALN_PER_SECOND;
    ...
    PARAMETER.needToUpdateAsset =
        config.hasPath(Constant.STORAGE_NEEDTO_UPDATE_ASSET) ? config
            ...
            : true;
    PARAMETER.alnReferenceBlock = config.hasPath(Constant.ALN_REFERENCE_BLOCK)
        ? config.getString(Constant.ALN_REFERENCE_BLOCK) : "head";

    PARAMETER.alnExpirationTimeInMilliseconds =
        config.hasPath(Constant.ALN_EXPIRATION_TIME_IN_MILLIS_SECONDS)
            && config.getLong(Constant.ALN_EXPIRATION_TIME_IN_MILLIS_SECONDS) > 0
            ? config.getLong(Constant.ALN_EXPIRATION_TIME_IN_MILLIS_SECONDS)
    ...
    PARAMETER.alnCacheEnable = config.hasPath(Constant.NODE_RPC_ALN_CACHE_ENABLE)
            && config.getBoolean(Constant.NODE_RPC_ALN_CACHE_ENABLE);
