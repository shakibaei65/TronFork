...
@Slf4j(topic = "API")
public class AloneJsonRpcImpl implements AloneJsonRpc {
  ...
  public AloneJsonRpcImpl(NodeInfoService nodeInfoService, Wallet wallet, Manager manager) {
  ...
  @Override
  public String getAlnBalance(String address, String blockNumOrTag)
      throws JsonRpcInvalidParamsException {
  ...
  private void callTriggerConstantContract(byte[] ownerAddressByte, byte[] contractAddressByte,
      long value, byte[] data, TransactionExtention.Builder alnExtBuilder,
    ...
    TransactionCapsule alnCap = wallet.createTransactionCapsule(triggerContract,
        ContractType.TriggerSmartContract);
    Transaction aln =
        wallet.triggerConstantContract(triggerContract, alnCap, alnExtBuilder, retBuilder);
  ...
  public String estimateGas(CallArguments args) throws JsonRpcInvalidRequestException,
    ...
    try {
      ...
      return ByteArray.toJsonHex(alnExtBuilder.getEnergyUsed());
  ...
  private TransactionJson buildTriggerSmartContractTransaction(byte[] ownerAddress,
    ...
    try {
      ...
      Transaction aln = wallet
          .triggerContract(build.build(), new TransactionCapsule(txBuilder.build()), alnExtBuilder,
              retBuilder);
      aln = setTransactionPermissionId(args.permissionId, aln);
      alnExtBuilder.setTransaction(aln);
    } catch (JsonRpcInvalidParamsException e) {
      throw new JsonRpcInvalidParamsException(e.getMessage());
    } catch (ContractValidateException e) {
    ...
    String jsonString = Util.printTransaction(alnExtBuilder.build().getTransaction(), args.visible);
    TransactionJson transactionJson = new TransactionJson();
