...
@Slf4j(topic = "DB")
public abstract class AloneStoreWithRevoking<T extends ProtoCapsule> implements IAloneChainBase<T> {
  ...
  protected AloneStoreWithRevoking(String dbName) {
  ...
  protected AloneStoreWithRevoking(DB<byte[], byte[]> db) {
    int dbVersion = CommonParameter.getInstance().getStorage().getDbVersion();
    if (dbVersion == 2) {
  ...
  // only for test
  protected AloneStoreWithRevoking(String dbName, RevokingDatabase revokingDatabase) {
    this.revokingDB = new RevokingDBWithCachingOldValue(dbName,
        (AbstractRevokingStore) revokingDatabase);
  }

  // only for test
  protected AloneStoreWithRevoking(String dbName, Options options,
