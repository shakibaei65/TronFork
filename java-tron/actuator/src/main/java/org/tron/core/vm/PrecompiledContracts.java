...
public class PrecompiledContracts {
  ...
  public static class ValidateMultiSign extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] rawData) {
      ...
      byte[] combine = ByteUtil
          .merge(convertToAloneAddress(addr), ByteArray.fromInt(permissionId), data);
      ...
      AccountCapsule account = this.getDeposit().getAccount(convertToAloneAddress(addr));
      if (account != null) {
        try {
  ...
  public static class RewardBalance extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      long rewardBalance =
          VoteRewardUtil.queryReward(convertToAloneAddress(callerAddress), getDeposit());
  ...
  public static class IsSrCandidate extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      WitnessCapsule witnessCapsule = this.getDeposit()
          .getWitness(convertToAloneAddress(addr));
  ...
  public static class VoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      DataWord[] words = DataWord.parseArray(data);
      byte[] voteAloneAddr = convertToAloneAddress(words[0].getLast20Bytes());
      byte[] targetAloneAddr = convertToAloneAddress(words[1].getLast20Bytes());

      long voteCount = 0;
      AccountCapsule voteAccountCapsule = this.getDeposit().getAccount(voteAloneAddr);
      if (voteAccountCapsule != null && !voteAccountCapsule.getVotesList().isEmpty()) {
        ...
        for (Protocol.Vote vote : voteList) {
          if (ByteString.copyFrom(targetAloneAddr).equals(vote.getVoteAddress())) {
  ...
  public static class ReceivedVoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      DataWord[] words = DataWord.parseArray(data);
      byte[] targetAloneAddr = convertToAloneAddress(words[0].getLast20Bytes());

      long voteCount = 0;
      WitnessCapsule witnessCapsule =
          this.getDeposit().getWitness(targetAloneAddr);
      if (witnessCapsule != null) {
  ...
  public static class TotalVoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      AccountCapsule accountCapsule = this.getDeposit().getAccount(voteAloneAddr);
      if (accountCapsule != null) {
        return Pair.of(true,
            longTo32Bytes(accountCapsule.getTronPower() / ALN_PRECISION));
      }
