...
import org.tron.core.exception.AloneException;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send ALN to.
   * @param value the ALN value transferred in the internal transaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = TransactionTrace.convertToAloneAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = TransactionTrace.convertToAloneAddress(obtainerAddress.getLast20Bytes());
      ...
      try {
        ...
        if (VMConfig.allowTvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all aln failed in suicide: %s", e.getMessage());
        }
  ...
  private void transferDelegatedResourceToInheritor(byte[] ownerAddr, byte[] inheritorAddr, Repository repo) {
    ...
    repo.addTotalNetWeight(-frozenBalanceForBandwidthOfOwner / ALN_PRECISION);

    long frozenBalanceForEnergyOfOwner =
        ownerCapsule.getAccountResource().getFrozenBalanceForEnergy().getFrozenBalance();
    repo.addTotalEnergyWeight(-frozenBalanceForEnergyOfOwner / ALN_PRECISION);
...
//    return freezeCheck && voteCheck;
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    byte[] senderAddress = TransactionTrace
        .convertToAloneAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setAlnHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(
                TransactionTrace.convertToAloneAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = TransactionTrace
        .convertToAloneAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = TransactionTrace
        .convertToAloneAddress(getContractAddress().getLast20Bytes());
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer aln failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(
            TransactionTrace.convertToAloneAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit()
        .getCode(TransactionTrace.convertToAloneAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] aloneAddr = TransactionTrace.convertToAloneAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(aloneAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(aloneAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(aloneAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState()
        .getBalance(TransactionTrace.convertToAloneAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord getRewardBalance(DataWord address) {
    long rewardBalance = VoteRewardUtil.queryReward(
        TransactionTrace.convertToAloneAddress(address.getLast20Bytes()), getContractState());
    return new DataWord(rewardBalance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(TransactionTrace.convertToAloneAddress(address.getLast20Bytes()));
  ...
  public DataWord isSRCandidate(DataWord address) {
    WitnessCapsule witnessCapsule = getContractState()
            .getWitness(TransactionTrace.convertToAloneAddress(address.getLast20Bytes()));
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState()
        .getTokenBalance(TransactionTrace.convertToAloneAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    ...
    if(VMConfig.allowTvmIstanbul()) {
      ...
      senderAddress = TransactionTrace
          .convertToAloneAddress(this.getCallerAddress().getLast20Bytes());
    }
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(TransactionTrace.convertToAloneAddress(msg.getType().callIsDelegate()
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowTvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer trc10 or ALN to non-exist address(would create one)
  ...
  public boolean freeze(DataWord receiverAddress, DataWord frozenBalance, DataWord resourceType) {
    ...
    byte[] owner = TransactionTrace.convertToAloneAddress(getContractAddress().getLast20Bytes());
    byte[] receiver = TransactionTrace.convertToAloneAddress(receiverAddress.getLast20Bytes());
  ...
  public long freezeExpireTime(DataWord targetAddress, DataWord resourceType) {
    byte[] owner = TransactionTrace.convertToAloneAddress(getContractAddress().getLast20Bytes());
    byte[] target = TransactionTrace.convertToAloneAddress(targetAddress.getLast20Bytes());
    ...
    try {
      ...
      for (int i = 0; i < witnessArrayLength; i++) {
        ...
        DataWord amount = new DataWord(Arrays.copyOfRange(amountArrayData,
            i * DataWord.WORD_SIZE, (i + 1) * DataWord.WORD_SIZE));
        param.addVote(TransactionTrace.convertToAloneAddress(witness.getLast20Bytes()),
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(AloneException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(AloneException e) {
      return new PrecompiledContractException(e.getMessage());
    }
