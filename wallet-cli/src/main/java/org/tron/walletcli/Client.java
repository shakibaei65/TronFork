...
public class Client {
  ...
  private void assetIssue(String[] parameters)
    ...
    if (parameters == null || parameters.length < 12) {
      ...
      System.out.println("AssetIssue [OwnerAddress] AssetName AbbrName TotalSupply AlnNum AssetNum Precision "
      ...
      System.out.println("AlnNum and AssetNum represents the conversion ratio of the alone to the asset.");
    ...
    String alnNumStr = parameters[index++];
    ...
    int alnNum = new Integer(alnNumStr);
    ...
    boolean result = walletApiWrapper.assetIssue(ownerAddress, name, abbrName, totalSupply,
        alnNum, icoNum, precision, startTime, endTime, 0,
  ...
  private void freezeBalance(String[] parameters)
    ...
    if (parameters == null || !(parameters.length == 2 || parameters.length == 3
      ...
      System.out.println("freezeBalance [OwnerAddress] frozen_balance frozen_duration "
          + "[ResourceCode:0 BANDWIDTH,1 ENERGY,2 ALONE_POWER] [receiverAddress]");
      return;
    }
  ...
  private void unfreezeBalance(String[] parameters)
    ...
    if (parameters == null || parameters.length < 1 || parameters.length > 3) {
      ...
      System.out.println(
          "unfreezeBalance [OwnerAddress] ResourceCode(0 BANDWIDTH,1 ENERGY,2 ALONE_POWER) [receiverAddress]");
      return;
    }
...
//  }
  ...
  private void deployContract(String[] parameter)
    ...
    if (parameters == null ||
      ...
      System.out.println(
          "DeployContract [ownerAddress] contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent origin_energy_limit value token_value token_id(e.g: ALNTOKEN, use # if don't provided) <library:address,library:address,...> <lib_compiler_version(e.g:v5)>");
//      System.out.println(
//          "Note: Please append the param for constructor tightly with byteCode without any space");
  ...
  private void triggerContract(String[] parameters, boolean isConstant)
    ...
    if (isConstant) {
      ...
      if (parameters == null || (parameters.length != 8 && parameters.length != 9)) {
        ...
        System.out.println(cmdMethodStr + " [OwnerAddress] contractAddress method args isHex"
            + " fee_limit value token_value token_id(e.g: ALNTOKEN, use # if don't provided)");
        return;
      }
    }
  ...
  private void listShieldedNote(String[] parameters) {
    ...
    if (showType == 0) {
      ...
      for (Entry<Long, ShieldedNoteInfo> entry : noteMap.entrySet()) {
        ...
        string += " ";
        string += entry.getValue().getAlnId();
        string += " ";
  ...
  private void help() {
    System.out.println("Help: List of Alone Wallet-cli commands");
    System.out.println(
  ...
  private void run() {
    System.out.println(" ");
    System.out.println("Welcome to Alone Wallet-Cli");
