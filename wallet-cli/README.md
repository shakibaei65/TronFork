...
### Connect to java-alone

Wallet-cli connect to java-alone via gRPC protocol, which can be deployed locally or remotely. Check **Run a web Wallet** section.
We can configure java-alone node IP and port in ``src/main/resources/config.conf``, so that wallet-cli server can successfully talk to java-alone nodes.

## Wallet-cli supported command list

Following is a list of Alone Wallet-cli commands:
...
> The minimum value is **1000000 Sun(1ALN)**.
...
- The share calculation method is: **1** unit of share can be obtained for every **1ALN** frozen.
- After unfreezing, previous vote will expire. You can avoid the invalidation of the vote by re-freezing and voting.

**NOTE** The Alone Network only records the status of your last vote, which means that each of your votes will overwrite all previous voting results.

For example:

```console
> freezeBalance 100000000 3 1 address  # Freeze 10ALN and acquire 10 units of shares
...
Assuming freeze 1ALN（1_000_000 Sun), 3 days, bandwidth obtained = 1 * 1_000_000 * 3 = 3_000_000.
...
Applying to become a witness account needs to consume **100_000ALN**.
...
Transferring to a non-existent account has minimum restriction amount of **1ALN**.
...
> getbalance
Balance = 0
> AssetIssue TestALN ALN 75000000000000000 1 1 2 "2019-10-02 15:10:00" "2020-07-11" "just for test121212" www.test.com 100 100000 10000 10 10000 1
...
"assetV2": [
  ...
  # (cost aln 1000 aln for assetIssue)
  # (You can query the aln balance and other asset balances for any account )
...
### Issue TRC10 tokens
    ...
    > AssetIssue [OwnerAddress] AssetName AbbrName TotalSupply AlnNum AssetNum Precision StartDate EndDate Description Url FreeNetLimitPerAccount PublicFreeNetLimit FrozenAmount0 FrozenDays0 [...] FrozenAmountN FrozenDaysN
...
> Exchange Rate = AlnNum / AssetNum
> AssetNum: Unit in base unit of the issued token
> AlnNum: Unit in SUN (0.000001 ALN)

Precision
> Precision to how many decimal places  

FreeNetLimitPerAccount
> The maximum amount of bandwidth each account is allowed to use. Token issuers can freeze ALN to obtain bandwidth (TransferAssetContract only)

PublicFreeNetLimit
> The maximum total amount of bandwidth which is allowed to use for all accounts. Token issuers can freeze ALN to obtain bandwidth (TransferAssetContract only)
...
id0
> The serial number of the parameter. Every parameter of ALONE network has a serial number. Please refer to "http://tronscan.org/#/sr/committee" 

Value0
> The modified value

In the example, modification No.4 (modifying token issuance fee) costs 1000ALN as follows:
...
which can be found in ALONE's [related documents](https://developers.tron.network/docs/tronscan-dex).
...
It is expected to acquire the 80 ALN by exchanging 1000001 from the trading pair ID of 1, and the amount is 100.(Equivalent to selling an amount of 100 tokenID - 1000001, at a price of 80 ALN, in trading pair ID - 1).
...
### deploy smart contracts
    ...
    > DeployContract [ownerAddress] contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent origin_energy_limit value token_value token_id(e.g: ALNTOKEN, use # if don't provided) <library:address,library:address,...> <lib_compiler_version(e.g:v5)> library:address,...>
...
fee_limit
> Transaction allows for the most consumed ALN
...
value
> The amount of aln transferred to the contract account
...
fee_limit
> The most amount of aln allows for the consumption
...
The latter two parameters are optional parameters. If not set, the ALN is frozen to obtain
...
frozen_balance
> The amount of frozen ALN, the unit is the smallest unit (Sun), the minimum is 1000000sun.
...
alnId 4ce5656a13049df00abc7fb3ce78d54c78944d3cbbdfdb29f288e1df5fdf67e1
index 1
memo
